...
func spawn_starter_cart() -> void:
	...
	mast.set_meta("world_anchor", true)
	mast.freeze = true
	...
	var _br := axle_wheel.get_connector_global("a1").origin.distance_to(mast.get_connector_global("h4").origin)
	print("STARTER bearing_residual_m=", _br)
	...
	GameState.notify("스타터: 모터→기어→바퀴 (베어링 1) joints=%d" % jn)
