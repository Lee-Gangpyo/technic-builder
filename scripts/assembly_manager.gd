...
func spawn_starter_cart() -> void:
	## Emergency Drive P0: single-axle direct drive (motor → axle → wheels).
	## #17 dual-gear + double bearings overconstrained on Play; #18 joint-spam forbidden.
	## One island, 4 joints, each pair connected once. Chassis is world_anchor in Drive.
	## Gear reduction returns Day2 with a non-overconstrained graph.
	clear_all(false)
	var y: float = 3.2
	...
	for p in parts:
		p.freeze = true
	GameState.notify("스타터: 모터→액슬→바퀴 (Drive 안정)")
